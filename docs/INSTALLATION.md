...
The optional host requires a dedicated non-root Linux user, independent identity, exact systemd/polkit policy and shared registry directory: follow [HOST_AGENT](HOST_AGENT.md). Production Ubuntu 24.04 ARM64, cloud, rclone and stability checks are required by [VALIDATION](VALIDATION.md). For rc.2 preserve identity/configuration and follow [MIGRATION](MIGRATION.md).
