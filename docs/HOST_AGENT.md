...
4. Create `/var/lib/plexonpanel-host` and `/var/backups/plexonpanel`, host-owned mode 0700. The Host keeps its durable authorization mirror at `/var/lib/plexonpanel-host/access/devices.json`; the directory is 0700 and mirror/lock files are 0600. Backups must be outside the live root. Configure only needed top-level include names.
