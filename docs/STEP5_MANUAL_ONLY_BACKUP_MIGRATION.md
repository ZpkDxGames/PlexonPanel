...
Existing values may remain on disk during migration, but they are non-authoritative. The full-restore-point schedule is no longer part of the runtime settings model; when maintenance settings are saved again, that legacy JSON member is omitted. No interval scheduler or calendar full-backup scheduler is started. Restart scheduling remains independent and may stay enabled.

For upgrade compatibility, the Host configuration parser may still accept legacy `backups.intervalMinutes` and live-snapshot exclusion fields from an older config file. They are inert: no live-snapshot engine or recurring-backup executor is constructed, and those fields cannot schedule work. They are removed from shipped examples and may be deleted from the local Host configuration after the matched release is installed.
...
4. The Host stops the configured systemd service and proves it is inactive/failed with no live MainPID before archiving.
5. The Host creates and locally verifies the cold restore point.
6. The Host applies the Google Drive/rclone upload and failure policy from Step 4.
7. The Host restarts Minecraft where required and verifies systemd plus the Host-local RCON readiness probe.

The Step 4 remote-progress contract is preserved: `UPLOADING_REMOTE` begins at zero uploaded bytes and reaches the verified archive byte count only after the remote promotion/verification phase succeeds. The Host never fabricates transfer progress from local archive bytes or forwards raw rclone output.

The Paper plugin does not participate in these backup phases and may be disabled or disconnected while the Host performs them.
