...
/** One console classifier/fingerprint implementation shared by every console producer. */
public final class ConsoleClassifier {
  ...
  public Classification classify(String line) {
    return classify(line, null);
  }

  public Classification classify(String line, Integer journalPriority) {
    ...
    String journalLevel =
        journalPriority == null
            ? "INFO"
            : journalPriority <= 3 ? "ERROR" : journalPriority == 4 ? "WARN" : "INFO";
    String level = stronger(contentLevel, journalLevel);
    ...
    if ("ERROR".equals(level) || "WARN".equals(level)) {
      ...
      normalized =
          WHITESPACE.matcher(normalized).replaceAll(" ").strip().toLowerCase(Locale.ROOT);
      fingerprint = Hashing.sha256Hex(normalized).substring(0, 24);
    }
    return new Classification(level, fingerprint);
  }

  private static String stronger(String content, String journal) {
    if ("ERROR".equals(content) || "ERROR".equals(journal)) return "ERROR";
    if ("WARN".equals(content) || "WARN".equals(journal)) return "WARN";
    if ("DEBUG".equals(content)) return "DEBUG";
    return "INFO";
  }
