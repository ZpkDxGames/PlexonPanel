...
public final class HostMain {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (!Files.isRegularFile(Path.of(config.accessRegistry()), LinkOption.NOFOLLOW_LINKS))
      throw new IllegalStateException("The configured access registry does not exist");
    ...
    MaintenanceManager maintenance =
        new MaintenanceManager(config, service, commands, operationLock, audit, connection, fullBackups);
              ...
              switch (action) {
                ...
                case "backup.preflight" -> {
                  Map<String, Object> result =
                      new LinkedHashMap<>(fullPreflight.check(maintenance.settings().fullRestorePoint()));
