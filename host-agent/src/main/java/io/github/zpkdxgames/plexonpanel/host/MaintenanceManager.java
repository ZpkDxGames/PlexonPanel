...
public final class MaintenanceManager implements AutoCloseable {
  ...
  MaintenanceManager(
    ...
    // Step 5 migration: any persisted full-backup schedule is made inert immediately.
  ...
  public void start() {
    try {
      ...
      if (active != null && "COUNTDOWN".equals(active.phase())) {
        try {
          ...
          // Invalid legacy countdown state is handled by the durable recovery classifier below.
        }
      }
      if (!resumedCountdown) {
    ...
    // Only restart scheduling survives Step 5. Full backups are explicit/manual operations.
  ...
  public Map<String, Object> status() throws Exception {
    ...
    Instant restart = MaintenanceSchedule.next(current.restart().schedule(), zone, timeSource.instant());
    MaintenanceStateStore.Job active = state.active();
  ...
  public String fullRestorePointNow(DeviceRegistry.Device device, boolean ignoredSkipCountdown)
      throws Exception {
    // The full-backup warning schedule is a Host safety invariant and cannot be skipped by clients.
  ...
  private void run(
    ...
    if ("RESTART".equals(job.kind()))
      ...
      runFull(job, device, automatic, false, recoveringCountdown);
    else
      fail(
          job,
          device,
  ...
  private void runRestart(
    ...
    try {
      ...
      audit(
          "maintenance.restart", "SUCCESS", actor(device, automatic), automatic, job.jobId(), "", Map.of());
    } catch (Exception error) {
  ...
  private void runFull(
    ...
    AtomicBoolean stopBoundaryEntered = new AtomicBoolean();
    try {
      if (automatic) throw new IllegalStateException("AUTOMATIC_BACKUP_DISABLED");
      if (!operationLock.tryLock()) throw new SecurityException("BUSY");
      locked = true;
      if (fullBackups.recoveryRequired()) throw new IllegalStateException("RESTORE_RECOVERY_REQUIRED");
      MaintenanceSettings current = settings;
      ...
      boolean localVerified =
          backup.local()
              ...
              && backup.verification() != null
              && backup.verification().contains("VERIFIED");
  ...
  private static MaintenanceSettings manualOnly(MaintenanceSettings value) {
    ...
    MaintenanceSettings.Schedule disabled =
        new MaintenanceSettings.Schedule(
            ...
            full.schedule() == null ? "04:00" : full.schedule().time());
