...
public record PanelSettings(
  ...
  public static PanelSettings load(FileConfiguration config) {
    ...
    Gateway gateway =
        new Gateway(
            ...
            bounded(
                config.getInt("gateway.connect-timeout-seconds", 10),
                2,
                60,
                "gateway.connect-timeout-seconds"),
            bounded(
                config.getInt("gateway.heartbeat-seconds", 15),
                5,
                120,
                "gateway.heartbeat-seconds"),
            bounded(
                config.getInt("gateway.reconnect.initial-delay-seconds", 2),
                1,
                60,
                "gateway.reconnect.initial-delay-seconds"),
            bounded(
                config.getInt("gateway.reconnect.maximum-delay-seconds", 60),
                5,
                900,
                "gateway.reconnect.maximum-delay-seconds"),
            bounded(
                config.getInt("gateway.queue-capacity", 2048),
                64,
                65_536,
                "gateway.queue-capacity"));
    if (gateway.initialReconnectDelaySeconds() > gateway.maximumReconnectDelaySeconds()) {
      throw new IllegalArgumentException(
          "gateway reconnect initial delay cannot exceed maximum delay");
    }
    if (gateway.enabled()
        && (!gateway.requireSignedMessages() || gateway.publicKeyBase64().isBlank())) {
      throw new IllegalArgumentException(
          "enabled gateways require signed messages and a pinned gateway.public-key");
    }
    ...
    Telemetry telemetry =
        new Telemetry(
            ...
            bounded(
                config.getLong("telemetry.server-interval-ticks", 100),
                20,
                72_000,
                "telemetry.server-interval-ticks"),
            bounded(
                config.getLong("telemetry.system-interval-seconds", 10),
                1,
                3600,
                "telemetry.system-interval-seconds"),
            bounded(
                config.getLong("telemetry.plugin-interval-seconds", 60),
                10,
                86_400,
                "telemetry.plugin-interval-seconds"),
    ...
    Console console =
        new Console(
            ...
            bounded(
                config.getLong("console.poll-interval-millis", 250),
                100,
                5000,
                "console.poll-interval-millis"),
            bounded(
                config.getLong("console.batch-interval-millis", 300),
                100,
                5000,
                "console.batch-interval-millis"),
            bounded(config.getInt("console.batch-size", 100), 1, 1000, "console.batch-size"),
            bounded(
                config.getInt("console.ring-buffer-lines", 1000),
                100,
                100_000,
                "console.ring-buffer-lines"),
            bounded(
                config.getInt("console.maximum-line-bytes", 16_384),
                512,
                1_048_576,
    ...
    RemoteActions remoteActions =
        new RemoteActions(
            ...
            Duration.ofSeconds(
                bounded(
                    config.getLong("remote-actions.maximum-clock-skew-seconds", 30),
                    5,
                    300,
  ...
  public record RemoteActions(
      boolean enabled, Duration maximumClockSkew, RemoteConsole console, RemotePlayers players) {}

  public record RemoteConsole(
      boolean enabled, List<String> allowPatterns, List<String> denyPatterns) {}

  public record RemotePlayers(
      boolean message, boolean kick, boolean ban, boolean unban, boolean whitelist) {}
