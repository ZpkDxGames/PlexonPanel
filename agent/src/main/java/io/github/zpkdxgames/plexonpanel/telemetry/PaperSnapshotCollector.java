...
public final class PaperSnapshotCollector {
  ...
  public ServerSnapshot serverSnapshot() {
    ...
    List<Double> tps =
        Arrays.stream(rawTps).map(value -> Math.round(value * 100.0) / 100.0).boxed().toList();

    long[] rawTickTimes = server.getTickTimes();
    double[] tickMillis =
        Arrays.stream(rawTickTimes).mapToDouble(nanos -> nanos / 1_000_000.0).sorted().toArray();
    ...
    return new ServerSnapshot(
        ...
        tickMillis.length == 0 ? 0.0 : round(tickMillis[0]),
        round(p95),
        round(maximum),
  ...
  public List<PluginSnapshot> pluginSnapshots() {
    ...
    for (Plugin plugin : server.getPluginManager().getPlugins()) {
      if (result.size() >= 257) break;
      PluginMeta metadata = plugin.getPluginMeta();
      result.add(
          new PluginSnapshot(
              metadata.getName(),
              bounded(metadata.getVersion(), 128),
              metadata.getMainClass(),
              metadata.getAuthors().stream().limit(16).map(v -> bounded(v, 64)).toList(),
              bounded(metadata.getWebsite(), 512),
              plugin.isEnabled(),
              metadata.getPluginDependencies().stream().limit(32).map(v -> bounded(v, 64)).toList(),
              metadata.getPluginSoftDependencies().stream()
                  .limit(32)
                  .map(v -> bounded(v, 64))
                  .toList()));
    }
